...
module.exports = {
  ...
  Tx.aggregate([
    ...
    if (err) {
      ...
      return cb(data[0].total / 100000000);
    }
  });
},

create_cmc: function(coin, cb) {
  var newCmc = new Cmc({
    symbol: coin,
    rank: "-1",
  });

  newCmc.save(function(err) {
    if (err) {
      console.log(err);
      return cb();
    } else {
      console.log("initial CoinMarketCap entry created for %s", coin);
      return cb();
    }
  })
},

check_cmc: function(coin, cb) {
  Cmc.findOne({symbol: coin}, function(err, exists) {
    if(exists) {
      return cb(true);
    } else {
      return cb(false);
...
get_cmc: function(coin, cb) {
  Cmc.findOne({symbol: coin}, function(err, data) {
    if(data) {
      return cb(data);
    } else {
      return cb(null);
...
get_latest_masternodestats: function(coin, cb) {
  ...
  update_coinmarketcap_db: function(coin, cb) {
    coinmarketcap.get_data(coin, function(err, obj) {
      if (err !== null) {
        return cb(err);
      }

      Cmc.update({ symbol: coin }, {
        rank: obj.rank,
        price_usd: obj.price_usd,
        price_btc: obj.price_btc,
        volume_24h_usd: obj['24h_volume_usd'],
        market_cap_usd: obj.market_cap_usd,
        available_supply: obj.available_supply,
